...
- (https://applex250.github.io/qwen3.8_27b_web/)
...
| 区块 | 说明 |
| --- | --- |
| 招牌单品 | 5 款风味卡，可按「花果香 / 发酵甜 / 醇厚 / 国产限定 / 秋日限定」筛选；含风味条动画、烘焙度点、限量库存条、**逐款冲煮建议** |
...
| 礼品卡 | 可选面额的卡面视觉（演示） |
| 会员计划 | 三档订阅 + 邮箱领券（演示） |
| 评价轮播 / FAQ | 自动轮播（hover 暂停、**键盘 ←/→ 与触屏滑动切换**）/ 手风琴问答 + 底部咨询入口 |
| 联系 / 咨询弹窗 | 主题选择 + 留言表单（演示提交），承接页脚与 FAQ 入口 |
| 购物袋 | 侧滑抽屉：加购、增减数量、**localStorage 本地持久化**、**包邮进度条**、演示订单号结算、Toast 提示 |

## 技术要点

- 单文件 HTML：`<!DOCTYPE html>` → 一个 `<style>` + 一个顶层 `<script>`
- 原生 Web：`IntersectionObserver`（滚动显现 / 导航高亮 / 统计）、`requestAnimationFrame`、
  `conic-gradient`、`localStorage`（购物袋持久化）、`prefers-reduced-motion` 降级
- 无障碍与细节：跳转链接（skip link）、弹窗 `aria` 属性、Esc 关闭、`@media print` 打印样式
- 分享与元信息：内联 SVG favicon、`theme-color`、Open Graph / Twitter Card（og:image 指向 `preview.png`）
